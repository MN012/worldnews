...
// Countries by continent for detection in article text
...
function toggleBookmark(link, title, source, event) {
  ...
  saveBookmarks();
  // Re-render current view to update bookmark buttons
...
function setupKeyboardShortcuts() {
  document.addEventListener('keydown', (e) => {
    ...
    // Number keys 1-7 for continents
    ...
    // Arrow keys to cycle continents
...
async function loadContinents() {
  try {
    ...
    const fallback = CONTINENT_ORDER;
    renderContinentNav(fallback.map(id => ({
      id,
...
async function fetchNews(continent) {
  ...
  try {
    const res = await fetch(`/api/news/${continent}`);
    if (!res.ok) throw new Error('Failed to fetch');
    const data = await res.json();

    // Source badges
    badges.innerHTML = data.sources
    ...
    buildCountryFilter(data.articles);
    buildTrendingTopics(data.articles);
...
async function summarizeNews() {
  ...
  try {
    let url = `/api/summarize/${currentContinent}`;
    if (activeCountryFilter) {
      url += `?country=${encodeURIComponent(activeCountryFilter)}`;
    }
    const res = await fetch(url);

    if (!res.ok) {
      const err = await res.json();
      body.innerHTML = `<div class="summary-error">${escapeHtml(err.error || 'Failed to generate summary')}</div>`;
      resetSummarizeBtn();
      return;
    }
    ...
    while (true) {
      ...
      for (const line of lines) {
        ...
        try {
          ...
          if (parsed.error) {
            body.innerHTML += `<div class="summary-error">${escapeHtml(parsed.error)}</div>`;
            break;
          }
